...
A flat weighted average is exactly how our first version failed: a convincing-looking dark band plus sharp printing outvoted an explicit serial-number red flag, and a known fake scored "genuine". v2 fuses by rules, with one more distinction learned from a real false positive: **read evidence vs measured evidence**. Hard red flags — wrong wording, nonexistent denomination, dead UV — are *read* from the note and keep full veto power. The serial telescopic check is *measured* through a camera proxy (ink-height columns, angle/compression-sensitive), so a serial red **alone** can only force INCONCLUSIVE ("this photo can't determine authenticity"); it needs corroboration from print-noise, the CNN, or another red before it can drive a counterfeit call. Weak proxies and the CNN can escalate but never overturn; the genuine path requires structural + majority-proxy + CNN agreement. Every verdict names which checks fired and why.
