...
function App() {
  ...
  const navItems = [
    ...
    { id: 'sentinel', label: 'Sentinel Safety Map', icon: Map, component: SentinelMapView },
    { id: 'sentinel-citizen', label: 'Sentinel Citizen App', icon: Smartphone, component: SentinelCitizenApp },
    { id: 'district-analytics', label: 'District Analytics', icon: Award, component: DistrictAnalyticsView, minRole: 'SP' },
    { id: 'cctv-simulator', label: 'CCTV AI Simulator', icon: Camera, component: CCTVAnalyticsSimulator, minRole: 'SP' },
        ...
        <div className="w-full max-w-md glass-panel p-8 rounded-2xl border border-slate-200 shadow-lg">
          ...
          {!showMfaStep ? (
            <form onSubmit={handlePreLogin} className="space-y-5">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Select User Account & Role</label>
                <select
                  value={username}
                  onChange={(e) => {
