...
| DP.next | Apply the two new env vars on Render + confirm | **User** | Render dashboard → `kawach-police` → Environment → add `PYTHONUTF8=1` and `PYTHONUNBUFFERED=1` manually (don't rely on render.yaml auto-sync timing) → save (redeploys) → re-test a chat message |
