...
| DP.next | Update `GEMINI_API_KEY` on Render to the new key + remove `/api/nayak/_debug_env` once confirmed | **User** | Render dashboard → `kawach-police` → Environment → set `GEMINI_API_KEY` to `AIzaSyDpvy7a5rswWI6aSjdea0-nP_ijFpPzfoY` (matches `police/backend/.env`) → save (redeploys) → test on `kawach-two.vercel.app` (frontend already points `VITE_POLICE_API_URL` at the live Render URL) |
