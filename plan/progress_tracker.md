...
| DP.18 | **🔴 Demo-blocking: Gemini free tier is capped at 20 requests/DAY per Google Cloud PROJECT** (not per-key — rotating the key does not reset this) | **Blocked on user — billing decision needed** | 2026-07-19 — confirmed via direct API error: `quotaId: GenerateRequestsPerDayPerProjectPerModel-FreeTier, limit: 20`. Exhausted today by cumulative testing (mine + teammate's + user's). The `retryDelay` Google returns (17-40s) is a generic backoff suggestion, NOT a real daily-reset ETA — repeated confirmed via direct retest, still 429 after the suggested delay passed. **Recommended fix: enable billing on the Google Cloud project** (pay-as-you-go removes the 20/day ceiling; demo-scale usage costs cents-to-low-dollars) — a new project only buys a fresh 20/day pool, not a real fix, since real demo traffic will re-exhaust it |
