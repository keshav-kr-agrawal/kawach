...
Last updated: 2026-07-17 (Phases 0-3 AI-tasks done; **currency CNN trained & deployed — 91.9% test acc on n=1,352, per-denomination numbers in `Classifier/weights/currency/eval_report.json`**; Pipeline 7 fully implements the ET PS bullet; PII audit done)
...
| 4.2 | Train CNN + per-denomination accuracy eval | AI+MANUAL | **Done — model deployed** | 2026-07-17 final run (Kaggle T4): EfficientNet-B0, test n=1,352 → **91.9% acc, fake-precision 0.907, fake-recall 0.914, AUC 0.964** (corrected 2026-07-20 — an earlier eval_report.json carried inflated numbers pasted from a wrong/overfitted Kaggle run; all docs re-synced to the real eval). Per-denom: ₹10 93.5% (n=378), ₹20 92.7%, ₹50 92.4%, ₹100 93.2%, ₹200 92.1%, ₹500 93.9% (circulating set avg **93.0%**), **₹2000 85.1% (n=47 — weak spot, thin fake data, denomination withdrawn from circulation May 2023)**, unknown-denom bucket missed its few fakes. Deployed to `Classifier/weights/currency/` and verified locally: loads as `cnn+heuristic`, fused inference ~1.1s/img CPU. Also fixed en route: AUC orientation bug (fake=idx0), stratified-split crash, CUDA env mismatch (session restart). Deck rule: quote per-denomination, flag ₹2000 honestly |
...
| DP.17 | **🔴 Security incident: real Gemini key committed in plain text to this file (this row), pushed to public GitHub, auto-flagged by Google's leak scanner** | Fixed + rotated | 2026-07-19 — my error: wrote the literal key value into a DP.next row above instead of referencing `police/backend/.env`. Redacted here immediately on discovery, key rotated. Scanned all tracked files for both known keys + the Supabase DB password — this was the only leak found |
| DP.18 | **Resolved without billing**: Gemini free-tier quotas are per-MODEL, not just per-project — `gemini-2.5-flash`'s 20/day cap being exhausted doesn't touch other models' buckets | Done | 2026-07-19 — switched `GEMINI_MODEL` default to `gemini-flash-lite-latest` (self-updating alias, currently → `gemini-3.1-flash-lite`): confirmed via direct API test to have a separate, unexhausted, much higher-limit quota pool, AND full function-calling support intact. Billing is still worth enabling before a real demo as a safety margin, but no longer a hard blocker |
| DP.19 | Fixed model-specific bug surfaced by the switch: `gemini-flash-lite-latest` 400s on multi-hop tool calls unless `thoughtSignature` is echoed back on each replayed functionCall part (`gemini-2.5-flash` didn't enforce this) | Done | 2026-07-19 — fixed by replaying the full original response part instead of a stripped `{"functionCall": fn_call}` reconstruction. **Verified live**: the scam-message test case now resolves in a SINGLE turn with real BNS citations (§308 Extortion, §351 Criminal Intimidation), an area-incident check, and a fully-formed `propose_report` proposal — noticeably richer output than the previous model. Commit `79336d9` |
| DP.20 | **Currency pipeline v2 rebuilt** after user's real-world test fakes exposed a false-negative fusion bug (both scored LIKELY_GENUINE/HIGH) + no defense against non-note inputs (white sheet ⇒ forced fake-vs-real score) | Done | 2026-07-19 — staged pipeline per `plan/currency_pipeline_v2_plan.md`: quality gate (dim/blur/glare ⇒ INSUFFICIENT_QUALITY + retake tip), presence gate (⇒ NOT_A_CURRENCY_NOTE), shared single OCR pass, NEW text-integrity check (**caught both test fakes by their literal wording — "CHILDREN BANK" / "MANORANJAN"**), tiered fusion (structural veto > weak-proxy corroboration > advisory CNN with 0.30-0.70 dead band), confidence capped MEDIUM for single visible-light photos. Frozen 9-case suite `Classifier/test_currency_v2.py` ALL PASS: both fakes ⇒ LIKELY_COUNTERFEIT, real ₹200 ⇒ LIKELY_GENUINE incl. dim/glare/blur/partial variants never flagged fake. nayak.py verdict mapping updated (gate exits ⇒ neutral, never "suspicious"). **CNN retrain judged unnecessary** — domain gap documented, model demoted to advisory. Needs manual HF Space sync to go live |
