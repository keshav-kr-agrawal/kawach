...
export default function PoliceAppLayout({ user, onLogout, children }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-950 font-sans select-text">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          {filteredNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 transition-transform duration-200 group-hover:scale-105 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-blue-500'}`} />
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 border-r border-slate-800 flex-col z-20 shadow-lg h-full">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          {filteredNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 transition-transform duration-200 group-hover:scale-105 ${isActive ? 'text-white' : 'text-slate-450 group-hover:text-blue-500'}`} />
                <span className="text-xs">{item.label}</span>
              </button>
            );
          })}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative bg-slate-950 text-slate-100">
        ...
        {/* Top Header */}
        <header className="h-16 border-b border-slate-850 flex items-center justify-between px-6 bg-slate-900 z-10">
          <div className="flex items-center space-x-3">
            <button
              onClick={() => setSidebarOpen(true)}
              className="p-2 -ml-2 hover:bg-slate-850 rounded-xl text-slate-400 md:hidden hover:text-blue-500 transition-colors"
              title="Open menu"
            >
              <Menu className="w-5 h-5" />
            </button>
            <h2 className="text-xs font-bold text-slate-250 uppercase tracking-widest font-outfit">
              {NAV_ITEMS.find(item => item.id === activeTabId)?.label || 'Dashboard'}
            </h2>
          </div>
          
          <div className="flex items-center space-x-4">
            <div className="px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full text-[9px] font-bold text-blue-400 tracking-wide uppercase">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-slate-950 text-slate-100">
          {children}
        </div>
      </main>
