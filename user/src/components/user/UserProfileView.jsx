  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white font-sans text-slate-900 overflow-y-auto pb-24 select-text">
      ...
      <div className="p-5 space-y-6">
        ...
        {/* User Card Info */}
        <div className="bg-white border-2 border-[#ffd900] rounded-3xl p-6 space-y-4 shadow-xs relative overflow-hidden">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-[#ffd900] text-slate-950 font-black text-xl rounded-2xl flex items-center justify-center border border-slate-950/10 shadow-xs font-sora">
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <div className="bg-yellow-400/10 border border-yellow-400/20 p-3 rounded-2xl">
              ...
              <h4 className="text-xl font-black text-slate-950 font-sora mt-0.5">{myReports.length}</h4>
            </div>
            <div className="bg-yellow-400/10 border border-yellow-400/20 p-3 rounded-2xl">
              <span className="text-[9px] font-bold text-[#b08850] uppercase tracking-wider block">Saved Laws</span>
              <h4 className="text-xl font-black text-slate-950 font-sora mt-0.5">{safeBookmarkedLaws.length}</h4>
            </div>
          </div>
        </div>

        {/* Ghost Mode Privacy Toggle */}
        <div className="bg-white border border-yellow-400/20 p-4 rounded-2xl flex items-center justify-between shadow-xs">
          <div>
            <h4 className="font-black text-slate-950 text-xs font-sora">Ghost Mode Metadata Scrubbing</h4>
            <p className="text-slate-500 text-[10px] font-semibold mt-0.5">Strip GPS EXIF & device identifiers before dispatch</p>
          </div>
          <input
            type="checkbox"
            checked={ghostMode}
            onChange={(e) => setGhostMode(e.target.checked)}
            className="w-5 h-5 accent-[#ffd900] cursor-pointer"
          />
        </div>
            ...
            <div className="space-y-3">
              {myReports.length === 0 ? (
                ...
                myReports.map((report) => (
                  <div key={report.id} className="bg-white border border-yellow-400/20 p-4 rounded-2xl space-y-2 shadow-xs">
                    <div className="flex items-center justify-between">
                      <h4 className="font-black text-slate-950 text-xs font-sora">{report.title}</h4>
                      <span className="text-[9px] font-bold text-slate-950 bg-[#ffd900] px-2 py-0.5 rounded uppercase font-mono">
                        {report.status}
                      </span>
                    </div>
                    <p className="text-slate-500 text-[11px] font-semibold">{report.description}</p>
            ...
            <div className="space-y-3">
              {safeBookmarkedLaws.length === 0 ? (
                ...
                safeBookmarkedLaws.map((lawId) => (
                  <div key={lawId} className="bg-white border border-yellow-400/20 p-4 rounded-2xl flex items-center justify-between shadow-xs">
                    <div>
                      <h4 className="font-black text-slate-950 text-xs font-sora">BNS Rule Citation #{lawId}</h4>
                      <span className="text-[10px] text-slate-400 font-bold">Saved in Legal Portfolio</span>
                    </div>
                    <button
