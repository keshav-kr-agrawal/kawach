  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white font-sans text-slate-900 overflow-y-auto pb-24 select-text">
      ...
      <div className="p-5 space-y-8">
            ...
            {flashcards.map((card) => {
              ...
              return (
                <div
                  ...
                  className="bg-white border-2 border-[#ffd900] rounded-2xl p-5 shadow-xs cursor-pointer hover:border-yellow-500 transition-all min-h-[160px] flex flex-col justify-between"
                >
                  {!isFlipped ? (
                    <div>
                      ...
                      <h4 className="font-black text-slate-950 text-sm font-sora">{card.title}</h4>
                      <p className="text-slate-600 text-xs font-semibold mt-2 leading-relaxed">
                        {card.frontDescription}
                      </p>
                      <span className="text-[9px] font-bold text-yellow-700 uppercase tracking-wider mt-4 block">
                    ...
                    <div>
                      ...
                      <div className="mt-3 p-2.5 bg-yellow-50 rounded-xl border border-yellow-200 text-[10px] font-bold text-slate-900">
        ...
        <section className="bg-white border border-yellow-400/20 rounded-3xl p-6 space-y-4 shadow-xs">
          <h3 className="text-sm font-black text-slate-950 font-sora uppercase tracking-wider">
          ...
          <form onSubmit={handleSearchQuerySubmit} className="relative">
            <input 
              ...
              className="w-full bg-slate-50 border border-yellow-400/20 rounded-xl pl-4 pr-12 py-3.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#ffd900] font-semibold"
              style={{ minHeight: '44px' }}
            />
            <button
              type="submit"
              className="absolute right-2 top-2 p-2 bg-[#ffd900] rounded-lg text-slate-950 font-bold hover:bg-yellow-400"
            >
          ...
          {aiAnswer && !aiLoading && (
            <div className="bg-yellow-50/60 border border-yellow-200 rounded-2xl p-4 space-y-2">
              <h4 className="font-extrabold text-sm text-slate-900 font-sora">{aiAnswer.title}</h4>
              <p className="text-slate-700 text-xs leading-relaxed font-semibold">{aiAnswer.details}</p>
              <div className="pt-1 flex gap-2">
                {aiAnswer.citations.map((c, i) => (
                  <span key={i} className="px-2 py-0.5 bg-yellow-200 text-slate-900 text-[9px] font-bold rounded font-mono">
                    🔖 {c}
                  </span>
                ))}
              </div>
        ...
        <section className="space-y-3">
          <h3 className="text-xs font-black text-slate-950 font-sora uppercase tracking-wider">
          ...
          <div className="space-y-3">
            {categories.map((cat) => {
              ...
              return (
                <div key={cat.id} className="bg-white border border-yellow-400/20 rounded-2xl overflow-hidden shadow-xs">
                  <button
                    onClick={() => setExpandedCategory(isExpanded ? null : cat.id)}
                    className="w-full p-4 flex items-center justify-between hover:bg-yellow-50/50 transition-colors text-left"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{cat.icon}</span>
                      <h4 className="font-black text-slate-950 text-sm font-sora">{cat.title}</h4>
                    </div>
                    <span className="text-xs font-bold text-slate-400">{isExpanded ? '▲' : '▼'}</span>
                  </button>

                  {isExpanded && (
                    <div className="p-4 bg-slate-50/60 border-t border-yellow-100 space-y-3">
                      {cat.laws.map((l, idx) => (
                        <div key={idx} className="bg-white border border-yellow-400/20 p-3.5 rounded-xl space-y-2">
                          <h5 className="font-extrabold text-slate-950 text-xs font-sora">{l.title}</h5>
                          <p className="text-slate-600 text-xs font-semibold">{l.law}</p>
